...
class BaseSpider(scrapy.Spider):
    ...
    def generate_initial_requests(self, config):
        ...
        if has_placeholder:
            ...
            # Probing response
            for handler_data in config['templated_url_response_handlers']:
                resp_handler = None
            ...
            url_injectors = []
            for param in config['parameter_handlers']:
    ...
    def extract_and_store_csv(self, response, description):
        ...
        try:
            if config["table_attrs"] is None:
                ...
                extra_config = extra_config_parser(config["table_attrs"])
                parsing_html.content.html_detect_content(
                    f"{self.data_folder}/raw_pages/{hsh}.{file_format}",
                    ...
                    displayed_only=extra_config['table_displayed_only'], to_csv=config["save_csv"]
                )
            success = True
