...
class SeleniumSpider(scrapy.Spider):
    ...
    def __init__(self, *a, **kw):
        ...
        self.tables_text = {
            "Despesa Total com Pessoal": "Despesa_Total_com_Pessoal",
            "Exclusões da Despesa Total com Pessoal": "Exclusoes_da_Despesa_Total_com_Pessoal",
            "APURAÇÃO DO CUMPRIMENTO DO LIMITE LEGAL": "APURAÇÃO_DO_CUMPRIMENTO_DO_LIMITE_LEGAL",
        }
    ...
    def gen_base_url(self):
        ...
        missing_files = json.loads(f.read())
        f.close()
        ...
        # city code with specific years missing
        for miss in missing_files["year"]:
    ...
    def get_tables(self, city, driver, year, base_url):
        for org in range(3, 0, -1):
            attempt_at_org = 8
            while attempt_at_org:
                try:
                    self.select_org(driver, str(org))
                    # time.sleep(1)

                    self.logger.info(f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> At org {self.org_name[org]} - {attempt_at_org}")

                    self.prepare_path(city, year, self.org_name[org])

                    for month in range(1, 13):
                        attempt_at_month = 8
                        while attempt_at_month:
                            try:
                                ...
                                try:
                                    ...
                                    for tid in self.tables:
                                        self.logger.info(
                                            f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> {self.org_name[org]} {self.month_name[month]} não existe"
                                        )
                                ...
                                self.logger.info(f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> {self.org_name[org]} {self.month_name[month]}")
                                ...
                                attempt_at_table = 8
                                while attempt_at_table > 0:
                                    try:
                                        self.logger.info(
                                            f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> {self.org_name[org]} {self.month_name[month]}" +
                                            f" - {attempt_at_org},{attempt_at_month},{attempt_at_table}"
                                        )
                                        tds = driver.find_elements_by_tag_name("td")
                                        for td in tds:
                                            if td.text in self.tables_text:
                                                table_html = td.find_element_by_xpath("..").find_element_by_xpath("..").find_element_by_xpath("..").get_attribute('outerHTML')
                                                self.logger.info(
                                                    f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> {self.org_name[org]} {self.month_name[month]}" + 
                                                    f" - {attempt_at_org},{attempt_at_month},{attempt_at_table} - found {td.text}"
                                                )
                                                df = pandas.read_html(table_html)[0]
                                                df.to_csv(fname + self.tables_text[td.text] + ".csv", index=False)
                                        break
                                    except StaleElementReferenceException:
                                        attempt_at_table -= 1
                                        self.logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> stale at table attempt")
                                        time.sleep(1)
                                # time.sleep(0.3)
                                break
                            except StaleElementReferenceException:
                                attempt_at_month -= 1
                                self.logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> stale at month attempt")
                                time.sleep(1)
                    break    
                except StaleElementReferenceException:
                    attempt_at_org -= 1
                    self.logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> stale at org attempt")
                    time.sleep(1)
