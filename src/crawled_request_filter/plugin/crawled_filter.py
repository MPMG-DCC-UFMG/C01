...
class CrawledFilter(BaseHandler):
    ...
    crawl_filter = CrawledRequestFilter()

    def setup(self, settings):
        '''
        Setup redis and tldextract
        '''
        print('Hey man, how is going?')
    ...
    def handle(self, dict):
        ...
        if dict['filter_if_crawled'] and self.crawl_filter.filter(dict):
            # log success
            dict['parsed'] = True
            dict['valid'] = True

            self.logger.info('This crawl has already been made.', extra=dict)
            return
