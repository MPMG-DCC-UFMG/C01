...
from schedule.config import (ConfigDict,
                            Config,
...
class TestConfig(unittest.TestCase):
    ...
    def test_raise_exception_with_invalid_start_date(self):
        ...
        for invalid_input in (None, invalid_format_date, past_date_str):
            ...
            with self.assertRaises(ConfigValueError, msg=f'Should raise exception if start date is {invalid_input}'):
    ...
    def test_raise_exception_with_invalid_repeat_mode(self):
        ...
        with self.assertRaises(ConfigInvalidRepeatModeError, msg='Should raise exception if repeat mode is invalid'):
            Config.valid_config(self.config_dict)
    ...
    def test_raise_if_personalized_repeat_has_invalid_repeat_interval(self):
        ...
        with self.assertRaises(ConfigValueError, msg='Should raise exception if interval is not a int'):
            Config.valid_config(self.config_dict)

        self.config_dict['personalized_repeat']['interval'] = -1
        with self.assertRaises(ConfigValueError, msg='Should raise exception if interval is not greater than 0'):
            Config.valid_config(self.config_dict)
    ...
    def test_raise_if_personalized_repeat_has_invalid_repeat_mode(self):
        ...
        with self.assertRaises(ConfigInvalidRepeatModeError, msg='Should raise exception if mode is invalid'):
            Config.valid_config(self.config_dict)
    ...
    def test_raise_if_personalized_repeat_mode_has_invalid_weekly_config(self):
        ...
        with self.assertRaises(ConfigValueError, msg='Should raise exception if data is not a list'):
            Config.valid_config(self.config_dict)

        self.config_dict['personalized_repeat']['data'] = []

        with self.assertRaises(ConfigValueError, msg='Should raise exception if data is empty'):
            Config.valid_config(self.config_dict)

        self.config_dict['personalized_repeat']['data'] = [7]

        with self.assertRaises(ConfigValueError, msg='Should raise exception if data has invalid values'):
            Config.valid_config(self.config_dict)
    ...
    def test_raise_if_personalized_repeat_mode_has_invalid_monthly_config(self):
        ...
        with self.assertRaises(ConfigValueError, msg='Should raise exception if data is not a dict'):
            Config.valid_config(self.config_dict)

        self.config_dict['personalized_repeat']['data'] = {}
        with self.assertRaises(ConfigMissingFieldError, msg='Should raise exception if data is empty'):
            Config.valid_config(self.config_dict)
        ...
        for invalid_value in ['-1', 0, 32]:
            ...
            with self.assertRaises(ConfigValueError, msg=f'Should raise exception if value is {invalid_value}'):
                Config.valid_config(self.config_dict)
    ...
    def test_raise_if_personalized_repeat_mode_finish_has_invalid_config(self):
        ...
        for invalid_input in ('-100', 0):
            ...
            with self.assertRaises(ConfigValueError, msg=f'Should raise exception if value is {invalid_input}'):
                Config.valid_config(self.config_dict)
    ...
    def test_if_config_can_be_retrieved_from_db(self):
        self._fill_personalized_repeat()

        Config.valid_config(self.config_dict)

        config = Config()
        config.load_config(self.config_dict)

        config.save(self.db_session)

        config_from_db = self.db_session.query(Config).first()

        self.assertTrue(config_from_db == config, msg='Should be equal to the config loaded from the dict')